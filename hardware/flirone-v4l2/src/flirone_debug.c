...
void vframe(char ep[],char EP_error[], int r, int actual_length, unsigned char buf[], unsigned char *colormap) 
{
  ...
  if (debug_printed_frames < 40) {
    printf("DBG header frame=%u thermal=%u jpg=%u status=%u pointer=%d first=%02x %02x %02x %02x\n",
           FrameSize, ThermalSize, JpgSize, StatusSize, buf85pointer,
           buf85[0], buf85[1], buf85[2], buf85[3]);
    fflush(stdout);
  }
...
// find the max, min and RMS (not used yet) values of the array
  ...
  if (debug_printed_frames < 40) {
    int center_med_raw = (pix[59 * 160 + 79]+pix[59 * 160 + 80]+pix[60 * 160 + 79]+pix[60 * 160 + 80])/4;
    printf("DBG thermal filecount=%d min_raw=%d center_raw=%d max_raw=%d temp=%.1f/%.1f/%.1f maxxy=%d,%d\n",
           filecount, min, center_med_raw, max,
           raw2temperature(min), raw2temperature(center_med_raw), raw2temperature(max), maxx, maxy);
    fflush(stdout);
    debug_printed_frames++;
  }
