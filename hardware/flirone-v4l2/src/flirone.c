...
static void decode_lepton2_thermal(const unsigned char *thermal, unsigned short *pix)
{
  int x, y;
  ...
  for (y = 0; y < 60; ++y)
  {
    ...
    for (x = 0; x < 80; ++x) {
      unsigned short v = read_be16(&row[x * 2]);
      int yy = y * 2;
      int xx = x * 2;
...
//  printf("\n"); 
  ...
  if (ThermalSize == 10332)
  {
    ...
    decode_lepton2_thermal(&buf85[28], pix);
  }
  else if (ThermalSize >= 39360)
  {
    ...
    decode_g2_thermal(&buf85[28], pix);
  }
  else
  {
    if (!thermal_layout_reported)
